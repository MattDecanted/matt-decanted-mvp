...
// ---------- demo fallback (if no row for today) ----------
      ...
      try {
        ...
        if (uid) {
          ...
          const { data: lb } = await supabase
            ...
            .limit(5);
          if (Array.isArray(lb)) setLeader(lb as LeaderRow[]);
        }
      } catch (e: any) {
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-semibold text-neutral-900">
                    {(lesson?.word ?? DEMO_TERM) || "No lesson scheduled"}
                  </h2>
                  <div className="mt-2 text-neutral-600 leading-relaxed">
                    {lesson?.description ??
                      `“${DEMO_TERM}” describes a bright, youthful red core most common in younger red wines; with age it trends toward garnet at the rim.`}
                  </div>
