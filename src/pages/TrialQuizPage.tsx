...
export default function TrialQuizPage() {
  ...
  async function submit() {
    if (!quiz) return;
    // ensure all answered
    if (answers.some(a => a < 0)) { setMessage("Please answer all questions."); return; }

    // need user id
    const { data } = await supabase.auth.getUser();
    const uid = data?.user?.id;
    if (!uid) { setMessage("Please sign in to save your points."); return; }
    ...
    try {
      ...
      if (body?.alreadyAttempted) setMessage("Already completed today.");
      else if (body?.error) setMessage(`Error: ${body.error}`);
      else setMessage(`You got ${body.correct} correct — +${body.points} points!`);
    } catch (e:any) {
      console.error(e);
  ...
  return (
    <main className="p-6 max-w-3xl mx-auto">
      ...
      <div className="mt-6 flex items-center gap-3">
        <button onClick={submit} className="px-4 py-2 rounded border bg-black text-white">
          Submit
        </button>
